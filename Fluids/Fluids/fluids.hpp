...
struct FluidCube
{
    FluidCube(int size, float diffusion, float viscosity, float dt)
    {
        ...
        s = vector<float>(N * N * N);
        density = vector<float>(N * N * N);

        Vx = vector<float>(N * N * N);
        Vy = vector<float>(N * N * N);
        Vz = vector<float>(N * N * N);

        Vx0 = vector<float>(N * N * N);
        Vy0 = vector<float>(N * N * N);
        Vz0 = vector<float>(N * N * N);
    }
    ...
    void project(vector<float>& velocX, vector<float>& velocY, vector<float>& velocZ, vector<float>& p, vector<float>& div, int iter, int N)
    {
        for (int k = 1; k < N - 1; k++) {
            for (int j = 1; j < N - 1; j++) {
                for (int i = 1; i < N - 1; i++) {
    ...
    void advect(int b, vector<float>& d, vector<float>& d0, vector<float>& velocX, vector<float>& velocY, vector<float>& velocZ, float dt, int N)
    {
