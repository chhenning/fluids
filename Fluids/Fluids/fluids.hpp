...
FluidCube FluidCubeCreate(int size, float diffusion, float viscosity, float dt)
{
    ...
    cube.s = vector<float>(N * N * N);
    cube.density = vector<float>(N * N * N);

    cube.Vx = vector<float>(N * N * N);
    cube.Vy = vector<float>(N * N * N);
    cube.Vz = vector<float>(N * N * N);

    cube.Vx0 = vector<float>(N * N * N);
    cube.Vy0 = vector<float>(N * N * N);
    cube.Vz0 = vector<float>(N * N * N);
...
static void set_bnd(int b, vector<float>& x, int N)
{
    for (int j = 1; j < N - 1; j++) {
        for (int i = 1; i < N - 1; i++) {
...
static void lin_solve(int b, vector<float>& x, vector<float>& x0, float a, float c, int iter, int N)
{
    float cRecip = 1.f / c;
...
static void diffuse(int b, vector<float>& x, vector<float>& x0, float diff, float dt, int iter, int N)
{
    float a = dt * diff * (N - 2) * (N - 2);
    lin_solve(b, x, x0, a, 1 + 6 * a, iter, N);
}

static void project(vector<float>& velocX, vector<float>& velocY, vector<float>& velocZ, vector<float>& p, vector<float>& div, int iter, int N)
{
    for (int k = 1; k < N - 1; k++) {
        for (int j = 1; j < N - 1; j++) {
            for (int i = 1; i < N - 1; i++) {
...
static void advect(int b, vector<float>& d, vector<float>& d0, vector<float>& velocX, vector<float>& velocY, vector<float>& velocZ, float dt, int N)
{
